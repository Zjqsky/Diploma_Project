...
class AutoEnc_Tensor(object):
    def __init__(self,dataSet):
        ...
        self.training_epochs = 100  # 训练的周期100
        self.batch_size = 32  # 每一批次训练的大小32
        self.display_step = 1
    ...
    def train(self):
        ...
        for epoch in range(self.training_epochs):
            ...
            for i in range(total_batch):
                ...
                _, c= self.sess.run([optimizer, cost], feed_dict={self.X: batch_xs})
    ...
    def test(self):
        ...
        for i in range(self.examples_to_show):
            a[0][i].imshow(np.reshape(self.dataSet[i], (50, 50,3)))
    ...
    def predict(self):
        saver = tf.train.Saver()
        saver.restore(self.sess, "network/model/model.ckpt")
        encode = self.sess.run(
            self.encoder_op, feed_dict={self.X: self.dataSet})

        size = encode.shape[0]
        i=1
        while (i * 1000) < size:
            np.savetxt('DataSet/netFea'+ str(i-1) +'.csv', encode[(i-1)*1000:i*1000], delimiter=',')
            i = i + 1
        np.savetxt('DataSet/netFea' + str(i-1) + '.csv', encode[(i-1) * 1000:size], delimiter=',')
